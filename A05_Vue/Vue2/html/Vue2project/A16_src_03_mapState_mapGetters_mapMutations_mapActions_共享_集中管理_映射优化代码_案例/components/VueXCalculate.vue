<template>
  <div class="div">
    ...
    <div class="jslistlength">京东集合长度：{{ jdlistlength.length }}</div>
  </div>
</template> 
...
export default {
  ...
  methods: {
    ...
    /**映射到 this.$store.commit:"方法名"  对象写法(参数在标签参)*/
    ...mapMutations({ addition: "ADDITION", subtraction: "SUBTRACTION" }),
    /**映射到this.$store.commit【方法名】参数必须和方法名一样(参数在标签参)  数组写法*/
    ...
    /**映射到 this.$store.dispatch:"方法名"  对象写法(参数在标签参)*/
    //  ...mapActions({ evenaddition: "evenaddition", delayaddition: "delayaddition" }),
    /**映射到this.$store.dispatch【方法名】参数必须和方法名一样(参数在标签参)  数组写法*/
