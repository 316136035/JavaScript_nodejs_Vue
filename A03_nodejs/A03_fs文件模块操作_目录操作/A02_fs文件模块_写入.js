...
// fs.writeFile('/A02_fs文件模块.text',`瀛洲学士风流远` ,err=>{
...
// fs.writeFile('./A02_fs文件模块.text',`配置对象,{flag:'a'}` ,{flag:'a'},err=>{
//     // err 错误对象  成功的话 err为null
//     if(err){
...
// fs.appendFile('/A02_fs文件模块.text',`瀛洲学士风流远appendFile` ,err=>{
//     // err 错误对象  成功的话 err为null
//     if(err){
//         console.log("追加写入失败")
//     }
...
// fs.writeFileSync('/A02_fs文件模块.text',`瀛洲学士风流远Sync` )
// //同步写入文件，这种写法追加整个文件的内容（文件路径(文件不存在就会创建新的文件，文件存在覆盖)，"文件内容")
// fs.appendFileSync('/A02_fs文件模块.text',`appendFileSync` )
