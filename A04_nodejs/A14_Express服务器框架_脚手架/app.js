...
app.set('views', path.join(__dirname, 'views'));//设置ejs中间件的视图目录路径。
app.set('view engine', 'ejs');//服务设置的视图引擎是 ejs （要安装ejs模块）
app.use(logger('dev'));// Express中间件，它用于将请求日志记录到控制台。
app.use(express.json());//使用内置中间件解析json请求体
app.use(express.urlencoded({ extended: false }));//使用内置中间件解析url编码请求体
app.use(cookieParser());//使用中间件解析cookie请求体
//use调用内置静态资源中间件(设置静态资源的目录路径)
app.use(express.static(path.join(__dirname, 'public')));
app.use('/', indexRouter);//使用本地路由模块（可定义前序/）
app.use('/users', usersRouter);//使用本地路由模块（可定义前序/user/）
//错误处理中间件
